...
class CodeGeneratorApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        tk.Label(frame, text="日期（YYYYMMDD）：", font=(None, 11)).grid(row=0, column=0, sticky="w")
        self.date_var = tk.StringVar()
        tk.Entry(frame, textvariable=self.date_var, width=32).grid(row=0, column=1, sticky="w", pady=4)

        self.generate_button = tk.Button(frame, text="生成验证码", width=14, command=self.generate_code)
        self.generate_button.grid(row=1, column=1, sticky="w", pady=16)

        self.status_label = tk.Label(frame, text="请输入日期后生成。", fg="gray")
        self.status_label.grid(row=2, column=1, sticky="w")

        tk.Label(frame, text="生成的码：", font=(None, 11)).grid(row=3, column=0, sticky="nw", pady=(12, 0))
        ...
        self.code_entry.grid(row=3, column=1, sticky="w", pady=(12, 0))
