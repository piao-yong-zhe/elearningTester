...
class CodeGeneratorApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        self.password_entry = tk.Entry(frame, textvariable=self.password_var, show="*", width=32)
        self.password_entry.grid(row=0, column=1, sticky="w", pady=4)
    ...
    def generate_code(self):
        ...
        try:
            token = create_license(date_str, password)
            self.code_var.set(token)
